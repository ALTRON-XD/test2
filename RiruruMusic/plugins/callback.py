...
async def unban_ass(cli, CallbackQuery, _):
    ...
    if umm.privileges.can_restrict_members:
        try:
            await app.unban_chat_member(int(chat_id), int(user_id))
        except:
            return await CallbackQuery.answer(_["call_8"], show_alert=True)
        return await CallbackQuery.edit_message_text(_["call_9"].format(userbot.one.name, CallbackQuery.from_user.mention))
    else:
        return await CallbackQuery.answer(_["call_10"], show_alert=True)
